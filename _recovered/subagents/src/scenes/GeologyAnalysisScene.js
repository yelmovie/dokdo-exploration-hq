/* =========================================================================
   5페이지 - 바위섬 분석실 (GeologyAnalysisScene) / 지형 (샘플 5 대응)
   2026-07 3D 개편: 좌측 관찰 영역을 three.js 3D 지형 탐사로 교체.
   - 3D 바위섬(동도·서도)을 돌려 보며 발광 마커 4개를 눌러 단서 수집
   - 단서 4개를 모두 모아야 우측 문제 보드 잠금 해제 (스펙 게이팅)
   - 단면 보기 토글: 클리핑으로 섬을 잘라 내부 지층 관찰
   - WebGL 미지원 기기: 기존 이미지 + 클릭 단서 라벨로 동일한 흐름 유지
   ========================================================================= */
...
/* 지형 단서 4종 — id는 DokdoTerrain3D의 TERRAIN_MARKERS와 1:1.
   fx/fy 는 2D 폴백 이미지 위 라벨 위치(%) */
const GEOLOGY_CLUES = [
...
export default function GeologyAnalysisScene(ctx) {
  ...
  const frame = missionFrame(ctx, layer, cfg, {
    ...
    helpText: use3D
      ? "바위섬을 돌려 보며 반짝이는 단서 4개를 모으면 문제 보드가 열려요."
      : "관찰 자료의 단서 라벨 4개를 모두 누르면 문제 보드가 열려요.",
  });

  /* ---- 좌측: 관찰 영역 (3D 지형 탐사 / 2D 폴백) ---- */
  const island = el("div", { style: { ...pos(28, 150, 560, 300), zIndex: 4, borderRadius: "18px", overflow: "hidden", border: "3px solid #fff", boxShadow: "var(--shadow)",
    background: use3D ? "linear-gradient(180deg,#8ecdf2 0%,#c8ecff 62%,#7fb6dd 100%)" : "#bfe0f3" } });
  ...
  if (use3D) {
    ...
    island.appendChild(el("div", { style: titlePillStyle, text: "🧭 3D 지형 탐사" }));
    island.appendChild(el("div", { style: { position: "absolute", left: "10px", bottom: "10px", zIndex: 6, background: "rgba(255,255,255,.88)", color: "var(--navy)", fontWeight: "800", fontSize: "12px", padding: "4px 10px", borderRadius: "999px", boxShadow: "var(--shadow-sm)" }, text: "🖱️ 드래그 회전 · 휠 확대 · 반짝이는 단서 클릭" }));

    /* 단면 상태에서만 보이는 지층 설명 라벨(DOM) */
    ...
    const strataTips = el("div", { style: { position: "absolute", left: "12px", top: "78px", zIndex: 6, display: "none", flexDirection: "column", gap: "8px" } }, [
    ...
    /* 단면 보기 토글 버튼 */
    let sectionOn = false;
    const secBtn = button("단면 보기", { variant: "gold", icon: "⛰️", onClick: () => {
      sectionOn = !sectionOn;
      viewer.setSectionView(sectionOn);
      strataTips.style.display = sectionOn ? "flex" : "none";
      const lab = secBtn.querySelector("span:last-child");
      if (lab) lab.textContent = sectionOn ? "겉모습 보기" : "단면 보기";
    } });
    Object.assign(secBtn.style, { position: "absolute", right: "10px", bottom: "10px", zIndex: 6, padding: "6px 14px", fontSize: "13px" });
    island.appendChild(secBtn);
  } else {
    /* ---- 2D 폴백: 기존 이미지 + 클릭 단서 라벨 (동일한 게이팅 흐름) ---- */
  ...
  const lockOverlay = el("div", { style: { position: "absolute", inset: "0", zIndex: 40, borderRadius: "inherit",
    ...
    el("div", { style: { fontWeight: "700", fontSize: "14px", opacity: ".9", lineHeight: "1.5" },
      text: use3D ? "왼쪽 3D 바위섬을 돌려 보며 반짝이는 표시를 눌러 단서를 모아요" : "왼쪽 관찰 자료의 단서 라벨을 눌러 단서를 모아요" }),
  ]);
  ...
  function render() {
    ...
    const qc = quiz(q, { mode: "radio", confirmLabel: "정답 확인", confirmAlign: "center", onResult: (ok) => {
      if (!ok) return;
      if (qi < total - 1) {
