...
export default function CompletionGalleryScene(ctx) {
  ...
  /* ---- 좌측: 3D 회전 디오라마 (WebGL 지원 시) ---- */
  let dioramaBuilt = false;
  if (supportsWebGL()) {
    const dio = createDokdoDiorama3D({ root, width: 308, height: 300 });
    if (dio) {
      dioramaBuilt = true;
      const holder = el("div", { style: { position: "relative", width: "308px", height: "300px", background: "linear-gradient(180deg,#eaf4fc,#d8ecf9)" } }, [
        dio.el,
        el("div", {
          style: { position: "absolute", left: "0", right: "0", bottom: "6px", textAlign: "center", fontSize: "12.5px", fontWeight: "700", color: "var(--navy)", opacity: ".7", pointerEvents: "none" },
          text: "손가락으로 돌려서 살펴봐요",
        }),
      ]);
  ...
  // 디오라마가 있으면 소년 캐릭터는 우측으로 축소 재배치, 없으면 기존 위치 유지
  if (dioramaBuilt) placeAsset(layer, DOKDO.boyScout, { x: 822, y: 408, w: 180, h: 250, alt: "축하하는 탐험가 소년", z: 4 });
  else placeAsset(layer, DOKDO.boyScout, { x: 130, y: 330, w: 250, h: 340, alt: "축하하는 탐험가 소년", z: 5 });
